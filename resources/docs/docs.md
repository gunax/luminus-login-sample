...
The `home-routes` handler in the `login-sample.routes.home` namespace
...
The `home-page` function will in turn call the `login-sample.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `login-sample.handler` namespace:
...
Request middleware functions are located under the `login-sample.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `login-sample.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `login-sample.db.core` in some other namespace.
